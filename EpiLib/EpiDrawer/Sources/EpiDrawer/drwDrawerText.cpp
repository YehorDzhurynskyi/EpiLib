...
#include "EpiDrawer/drwDrawerText.cxx"
...
void drwDrawerText::CreateAtlas(const drwTextFace& face, epiU32 fontSize)
{
...
void drwDrawerText::CreateAtlas(const drwTextFace& face, epiU32 fontSize, const epiWChar* abc)
{
    m_TextAtlas = face.CreateRenderedAtlas(abc, fontSize);
}

void drwDrawerText::DrawText(const epiWChar* text, const epiVec2f& position, epiFloat textHeight, const Color& color, epiFloat z)
{
    ...
    {
        const drwTextRenderedAtlasGlyph* glyph = m_TextAtlas.GlyphOf(text[i]);
        if (glyph == nullptr)
        {
            continue;
        }
...
void drwDrawerText::SceneBegin()
{
    //m_VertexBufferMappingText.Map(gfxVertexBufferMapAccess::Write);
}

void drwDrawerText::SceneEnd(const drwCamera& camera)
{
