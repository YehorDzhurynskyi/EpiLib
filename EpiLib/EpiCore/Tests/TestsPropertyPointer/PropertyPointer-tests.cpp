...
TEST(PropertyPointer, Base)
{
    ...
    while (m)
    {
        for (const auto& p : m->GetClassData())
        {
            ...
            switch (p.GetTypeID())
            {
            case epiMetaTypeID_epiChar: ASSERT_EQ(pp.Get<epiChar>(), '0'); pp.Set<epiChar>('H'); ASSERT_EQ(pp.Get<epiChar>(), 'H'); break;
            case epiMetaTypeID_epiWChar: ASSERT_EQ(pp.Get<epiWChar>(), L'0'); pp.Set<epiWChar>(L'H'); ASSERT_EQ(pp.Get<epiWChar>(), L'H'); break;

            case epiMetaTypeID_epiBool: ASSERT_EQ(pp.Get<epiBool>(), false); pp.Set<epiBool>(true); ASSERT_EQ(pp.Get<epiBool>(), true); break;

            case epiMetaTypeID_epiByte: ASSERT_EQ(pp.Get<epiByte>(), 0); pp.Set<epiByte>(12); ASSERT_EQ(pp.Get<epiByte>(), 12); break;

            case epiMetaTypeID_epiFloat: ASSERT_NEAR(pp.Get<epiFloat>(), 0.0f, epiFloatingEqTolerance()); pp.Set<epiFloat>(-125.0f); ASSERT_NEAR(pp.Get<epiFloat>(), -125.0f, epiFloatingEqTolerance()); break;
            case epiMetaTypeID_epiDouble: ASSERT_NEAR(pp.Get<epiDouble>(), 0.0, epiFloatingEqTolerance()); pp.Set<epiDouble>(-0.035); ASSERT_NEAR(pp.Get<epiDouble>(), -0.035, epiFloatingEqTolerance()); break;

            case epiMetaTypeID_epiSize_t: ASSERT_EQ(pp.Get<epiSize_t>(), 0); pp.Set<epiSize_t>(550); ASSERT_EQ(pp.Get<epiSize_t>(), 550); break;

            // TODO: handle
            // case epiMetaTypeID_epiString: ASSERT_EQ(pp.Get<epiString&>(), epiDEBUG_ONLY("Empty")); pp.Set<epiString&>("Test text"); ASSERT_EQ(pp.Get<epiString&>(), "Test text"); break;
            // case epiMetaTypeID_epiWString: ASSERT_EQ(pp.Get<epiWString&>(), epiDEBUG_ONLY(L"Empty")); pp.Set<epiWString&>(L"Test Wtext"); ASSERT_EQ(pp.Get<epiWString&>(), L"Test Wtext"); break;

            case epiMetaTypeID_epiU8: ASSERT_EQ(pp.Get<epiU8>(), 0); pp.Set<epiU8>(32); ASSERT_EQ(pp.Get<epiU8>(), 32); break;
            case epiMetaTypeID_epiU16: ASSERT_EQ(pp.Get<epiU16>(), 0); pp.Set<epiU16>(158); ASSERT_EQ(pp.Get<epiU16>(), 158); break;
            case epiMetaTypeID_epiU32: ASSERT_EQ(pp.Get<epiU32>(), 0); pp.Set<epiU32>(2444212); ASSERT_EQ(pp.Get<epiU32>(), 2444212); break;
            case epiMetaTypeID_epiU64: ASSERT_EQ(pp.Get<epiU64>(), 0); pp.Set<epiU64>(298190129300); ASSERT_EQ(pp.Get<epiU64>(), 298190129300); break;
            case epiMetaTypeID_epiS8: ASSERT_EQ(pp.Get<epiS8>(), 0); pp.Set<epiS8>(-16); ASSERT_EQ(pp.Get<epiS8>(), -16); break;
            case epiMetaTypeID_epiS16: ASSERT_EQ(pp.Get<epiS16>(), 0); pp.Set<epiS16>(-1241); ASSERT_EQ(pp.Get<epiS16>(), -1241); break;
            case epiMetaTypeID_epiS32: ASSERT_EQ(pp.Get<epiS32>(), 0); pp.Set<epiS32>(89537); ASSERT_EQ(pp.Get<epiS32>(), 89537); break;
            case epiMetaTypeID_epiS64: ASSERT_EQ(pp.Get<epiS64>(), 0); pp.Set<epiS64>(213125125); ASSERT_EQ(pp.Get<epiS64>(), 213125125); break;
